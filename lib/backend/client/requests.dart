...
Future<FetchEventsResult> fetchEventsAPI(String? cursor) async{
  try{
    final response = await http.get(
      Uri.parse((cursor == null) ? "$usedServerURL/events" : "$usedServerURL/events?after=$cursor"),
    ).timeout(const Duration(seconds: 5));
    if (response.statusCode == 200){
      final body = jsonDecode(response.body);
      print(body["events"]);
      return FetchEventsResult((body["events"] as List? ?? []).map((e) => Event.fromJson(e)).toList(), body["cursor"]);
    } else {
